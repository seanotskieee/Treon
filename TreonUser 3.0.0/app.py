...
# ⭐ App feedback → save to CSV (local only, no Firebase)
@app.route('/feedback', methods=['POST'])
def feedback():
    data = request.get_json()
    user_id = data.get("user_id", "guest")
    feedback_rating = data.get("rating")
    feedback_text = data.get("comment")

    now = get_ph_time()

    # Save feedback to CSV (local only)
    file_exists = os.path.isfile("feedback.csv")
    with open("feedback.csv", mode="a", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow(["timestamp", "user_id", "rating", "comment"])
        writer.writerow([format_ph_time(now), user_id, feedback_rating, feedback_text])

    # Optional: also save to JSON for backup (like sessions)
    os.makedirs("sessions", exist_ok=True)
    json_filename = f"sessions/{user_id}_feedback_{now.strftime('%Y%m%d_%H%M%S')}.json"
    with open(json_filename, "w", encoding="utf-8") as f:
        json.dump({
            "timestamp": format_ph_time(now),
            "user_id": user_id,
    ...
    print(f"💾 Saved Locally in feedback.csv and {json_filename}")

    return jsonify({"status": "success", "message": "Feedback saved locally"}), 200
