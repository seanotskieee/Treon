...
// CORRECTED: Show color rating with precise distribution bar alignment
function showColorRating() {
  ...
  if (selectedBrand && selectedColor && selectedBrand !== "undefined") {
    ...
    // CORRECTED: Calculate percentage based on exact star alignment
    // For average rating 3.0: 3/5 = 60% width
    // For average rating 4.5: 4.5/5 = 90% width
    const average = colorRating.average || 0;
    const percentage = Math.min(100, (average / 5) * 100);
    
    const distributionFilled = document.getElementById('distribution-filled');
    distributionFilled.style.width = `${percentage}%`;
    ...
    const distributionFilled = document.getElementById('distribution-filled');
    distributionFilled.style.width = '0%';
  }
}
  ...
  validProducts.forEach((product, index) => {
    ...
    if (product.colors && Array.isArray(product.colors)) {
      ...
      validColors.forEach((color, colorIndex) => {
        ...
        // CORRECTED: Calculate percentage for precise star alignment
        const average = colorRating.average || 0;
        const percentage = Math.min(100, (average / 5) * 100);
          ...
          <div class="color-meta">
            ...
            <div class="rating-distribution">
              <div class="distribution-bar">
                <div class="distribution-filled" style="width: ${percentage}%"></div>
                <div class="distribution-markers">
                  <div class="distribution-marker"></div>
                  <div class="distribution-marker"></div>
                  <div class="distribution-marker"></div>
                  <div class="distribution-marker"></div>
                </div>
