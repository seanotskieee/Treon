document.addEventListener('DOMContentLoaded', () => {
  ...
  // Remove the first capture event listener (lines 8-33) and keep only the second one
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    star.addEventListener('click', () => {
      ...
      if (shadeId) {
        fetch('/rate', {
          ...
          body: JSON.stringify({ shade_id: shadeId, rating: value, user_id: currentUserId })
        })
        .then(res => res.json())
  ...
  // 📸 Capture + Preview Modal - KEEP ONLY THIS CAPTURE LISTENER
  const captureBtn = document.getElementById('captureButton');
