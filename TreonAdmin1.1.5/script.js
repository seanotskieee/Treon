...
function loadShadesForBrand(brandName) {
  ...
  shades.forEach(shade => {
    ...
    item.addEventListener('click', () => {
      ...
      // Log the shade selection for session tracking
      logShade(shade.id);
    });
  ...
  if (firstShadeBtn) {
    ...
    // Log the first shade selection for session tracking
    logShade(colorId);
  }
}
...
function goToPrivacy() {
  ...
  resetRatingStars();
  // End the session when going to privacy screen
  endSession();
}
...
// NEW: Session tracking functions from second version
