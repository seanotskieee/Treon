...
import { getColorRatings, clearAllRatings } from './ratings-service.js';
...
window.loadBrandColors = loadBrandColors;
window.showColorRating = showColorRating;
...
async function loadProducts() {
  try {
    ...
    // Find the maximum ID to set nextProductId
...
async function fetchRatings() {
  try {
    ...
    ratingsSnapshot.forEach((doc) => {
      ...
      // Process rating data (simplified)
      // In a real application, you would aggregate the ratings
    });
...
async function showColorRating() {
  const brandSelect = document.getElementById('brand-select');
  const colorSelect = document.getElementById('color-select');
  const selectedBrand = brandSelect.value;
  ...
  if (selectedBrand && selectedColor) {
    const result = await getColorRatings(selectedBrand, selectedColor);
    
    if (result.success) {
      // Update UI with the rating data
      document.querySelector('.large-rating').textContent = result.average.toFixed(1);
      document.querySelector('.rating-count').textContent = `(${result.count} ratings)`;
      
      // Update star display
      const stars = document.querySelectorAll('.large-star');
      const ratingValue = Math.round(result.average);
      ...
      if (total > 0) {
        for (let i = 5; i >= 1; i--) {
          const percentage = (result.distribution[i] / total) * 100;
          const segment = distributionBar.querySelector(`.distribution-segment[data-rating="${i}"]`);
          segment.style.width = `${percentage}%`;
          segment.setAttribute('data-count', result.distribution[i]);
        }
      }
    }
  } else {
...
async function syncProductsWithServer() {
  try {
    ...
    // Clear existing products (optional - you might want to update instead of replace)
    const existingProducts = await getDocs(collection(db, "products"));
    existingProducts.forEach((doc) => {
      batch.delete(doc.ref);
    });
    
    // Add all current products to the batch
    products.forEach(product => {
    ...
    // Commit the batch
    await batch.commit();
    console.log('✅ Products synced with Firestore');
...
async function clearRatings() {
  if (confirm('Are you sure you want to clear all ratings? This action cannot be undone.')) {
    const result = await clearAllRatings();
    ...
    if (result.success) {
      ...
      alert('Error clearing ratings: ' + result.error);
    }
  }
}