...
function addProduct() {
  ...
  meta.innerHTML = `
    ...
    <div class="stars"></div>
    <div><button class="like-btn">❤️ Like</button> <span class="like-count">0 Likes</span></div>
  `;
  ...
  const likeBtn = meta.querySelector('.like-btn');
  const likeCountEl = meta.querySelector('.like-count');
  let likes = 0;
  likeBtn.onclick = () => {
    likes++;
    totalLikes++;
    likeCountEl.textContent = `${likes} Like${likes > 1 ? 's' : ''}`;
    document.getElementById('total-values').textContent = totalLikes;
  };
