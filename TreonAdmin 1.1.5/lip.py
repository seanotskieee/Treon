...
# Initialize mediapipe once at module level
...
def capture_frame(rating=0):
    ...
    # Note: The daily session count is now updated in app.py's capture_route function
    # so we don't need to make an HTTP request here
...
def generate_frames():
    ...
    # Precompute lip polygon for faster rendering
    lip_polygon = None
    ...
    while True:
        try:
            ...
            if current_color and results and results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    if cv2.countNonZero(lip_mask) > (h * w) * 0.003:
                        ...
                        feathered = cv2.GaussianBlur(overlay, (11, 11), 0)
                        
                        # Create a mask for the lips
                        lip_mask_3ch = cv2.merge([lip_mask, lip_mask, lip_mask])
                        
                        # Blend the overlay with the original image
                        blended = cv2.addWeighted(image_display, 1.0, feathered, 0.4, 0)
                        
                        # Apply the lip color only to the mask area
                        image_display = np.where(lip_mask_3ch > 0, blended, image_display)
                    else:
