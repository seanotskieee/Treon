...
def set_color(color_id):
    global selected_color_key, current_session_start
...
def capture_frame():
    ...
    if ret:
        ...
        if current_session_start:
            elapsed = time.time() - current_session_start
        ...
        filename = f'captured_image_{timestamp}.jpg'  # Define the filename
        
        session_data = {
            "id": str(uuid.uuid4()),  # Unique ID for each session
        ...
        # Reset session timer for next capture
        current_session_start = time.time()
