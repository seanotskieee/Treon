...
def get_session_stats():
    try:
        ...
        daily_sessions = [s for s in sessions 
                         if datetime.strptime(s["datetime"], "%m/%d/%Y %H:%M").date() == today]
        daily_total = sum(parse_duration(s["duration"]) for s in daily_sessions)
        daily_avg = format_duration(daily_total / len(daily_sessions)) if daily_sessions else "0m 0s"
        
        # Calculate weekly average
        week_ago = today - timedelta(days=7)
        weekly_sessions = [s for s in sessions 
                          if datetime.strptime(s["datetime"], "%m/%d/%Y %H:%M").date() >= week_ago]
        weekly_total = sum(parse_duration(s["duration"]) for s in weekly_sessions)
        weekly_avg = format_duration(weekly_total / len(weekly_sessions)) if weekly_sessions else "0m 0s"
        
        # Find peak sessions
        session_counts = defaultdict(int)
        for session in sessions:
            date = datetime.strptime(session["datetime"], "%m/%d/%Y %H:%M").strftime("%m/%d/%Y")
