...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupMonthlyControls() {
    ...
    document.getElementById('saveRecordBtn').addEventListener('click', saveDailyRecord);
    document.getElementById('cancelRecordBtn').addEventListener('click', hideAddRecordModal);
    document.getElementById('closeRecordModal').addEventListener('click', hideAddRecordModal);
  ...
  // Show modal for adding daily records
  function showAddRecordModal() {
    document.getElementById('addRecordModal').classList.remove('hidden');
    // Set today's date as default
    const today = new Date().toISOString().split('T')[0];
    document.getElementById('recordDate').value = today;
    document.getElementById('recordCount').value = '1';
  }
  ...
  async function saveDailyRecord() {
    ...
    if (!date || isNaN(count) || count < 0) {
      alert('Please enter a valid date and count (must be positive number)');
      return;
    }
    ...
    if (selectedDate > today) {
      alert('Cannot add records for future dates. Please select today or a past date.');
      return;
    }

    try {
      const dailySessionsRef = db.collection("daily_sessions").doc(date);
      
      // Get current count or set to 0
      const doc = await dailySessionsRef.get();
      const currentCount = doc.exists ? doc.data().count : 0;
      
      // Update with new count (add to existing or set new)
      ...
      console.log(`✅ Manual record added for ${date}: ${newCount} sessions`);
      hideAddRecordModal();
      
      // Reload monthly data if the added record is in the current view
      const recordDate = new Date(date);
      if (recordDate.getMonth() === currentMonth && recordDate.getFullYear() === currentYear) {
        loadMonthlyData();
      }
      
      alert(`Successfully added ${count} sessions for ${date}`);
